...
De situatie kan zich voordoen dat een bericht een grootte krijgt die niet meer efficiënt door de gateways en adapters verwerkt kan worden. Ook kan het zich voordoen dat er behoefte bestaat aan het buiten de normale procesgang ('out-of-band') uitwisselen van grote hoeveelheden gegevens tussen systemen. In die gevallen zal dit “grote bericht” in een ander proces of operatie verstuurd moeten worden: middels de Digikoppeling Koppelvlakstandaard Grote Berichten. Dit is vergelijkbaar met de door Amazon ontwikkelde Simple Storage Service (ofwel S3) De volgende aanpak wordt dan gehanteerd:

- De verzender stelt een bestand samen uit (een deel van) de gegevens die normaliter in het “grote bericht” verzonden zou worden. Het resultaat wordt aangeduid met de term “groot bestand” of "Object". Merk op dat dit ook een “groot” xml bestand kan zijn, een CAD bestand, een PDF document, een ZIP bestand, et cetera.
...
- Uitwisseling van het grote bestand vindt plaats via een PULL of een PUSH principe.
  - Bij Het PULL principe biedt de verzender het groot bestand aan via een Grote Berichten File service aan de ontvanger.  Vergelijkbaar met S3: `GET /bucket/object`
  - Bij het PUSH principe stuurt de verzender het groot bestand naar de Grote Berichten File service van de ontvanger. Vergelijkbaar met S3: `PUT /bucket/object`

- De bestandsoverdracht is niet “betrouwbaar”; hiervoor dient de ontvanger aanvullende maatregelen te implementeren (retry-mechanisme, foutafhandeling). De Koppelvlakstandaard en de S3 API bevatten hiervoor handvatten. Toepassing van deze handvatten in concrete implementaties vallen buiten de scope van het koppelvlak.

> Merk op dat het stuurbericht naast metadata en authenticatie ook voorzien kan zijn van inhoudelijke informatie die al nodig is bij de verwerking van het bericht.

Dit document beschrijft welke gegevens er in de metadata opgenomen moeten worden en hoe het HTTP protocol gebruikt moet worden voor de overdracht van het grote bestand.

## Nieuw in deze versie

In deze versie wordt de Digikoppeling Grote Berichten S3 variant geïntroduceerd, naast de reeds bestaande varianten. We hebben ervoor gekozen de beschrijving van de S3 variant te integreren in de bestaande versie, omdat de voorwaarden en regels voor verschillende varianten vrijwel identiek zijn en we de technische werking zo beter kunnen vergelijken.

## Verzenden van Grote Berichten

### Pull Principe

Het principe is dat de verzender het grote bestand aanbiedt via een Grote Berichten File Service en een bericht stuurt aan de ontvanger dat het bericht geplaatst is, de ontvanger kan het bestand vervolgens ophalen.
...
- Stap 1: De verzender verstuurt het bericht (request) met de meta-data van het bestand, bijvoorbeeld naam, locatie, grootte etc. en krijgt als reactie hierop (response) een verwijzing naar de download.
- Stap 2: De ontvanger ontvangt het bericht (response) met de meta-data, en download en verwerkt vervolgens het bestand (PULL). Vergelijkbaar met S3: `GET /bucket/object`

Opmerking  
De verzender maakt hiervoor het te verzenden bestand gereed, eventueel wordt dit in meerdere delen gesplitst (multipart) als dit wenselijk is.

### Push Principe

Het principe is dat de verzender het grote bestand aanbiedt aan de Grote Berichten File Service van de ontvanger (via een upload / PUT) en een bericht stuurt aan de ontvanger dat het grote bestand verstuurd is, de ontvanger kan het bestand vervolgens verwerken.
...
- Stap 1: De verzender verstuurt (Push) het grote bericht naar de Grote berichten File service van de ontvanger. vergelijkbaar met S3: `PUT /bucket/object`
- Stap 2: De verzender verstuurt het bericht met de meta-data van het bestand, bijvoorbeeld naam, locatie, grootte etc.
...
Voor het gebruik van het Digikoppeling Koppelvlakstandaard Grote Berichten gelden onderstaande algemene eisen:
